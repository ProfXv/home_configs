...
struct speech_rec_notifier {
	void (*on_result)(const char *result, char is_last, void *user_data);
	void (*on_speech_begin)(void *user_data);
	void (*on_speech_end)(int reason, void *user_data);	/* 0 if VAD.  others, error : see E_SR_xxx and msp_errors.h  */
