...
static void end_sr_on_error(struct speech_rec *sr, int errcode)
{
	...
	if (sr->session_id) {
		if (sr->notif.on_speech_end)
			sr->notif.on_speech_end(errcode, sr->notif.user_data);
...
static void end_sr_on_vad(struct speech_rec *sr)
{
	...
	while(sr->rec_stat != MSP_REC_STATUS_COMPLETE ){
		...
		if (rslt && sr->notif.on_result)
			sr->notif.on_result(rslt, sr->rec_stat == MSP_REC_STATUS_COMPLETE ? 1 : 0, sr->notif.user_data);
	...
	if (sr->session_id) {
		if (sr->notif.on_speech_end)
			sr->notif.on_speech_end(END_REASON_VAD_DETECT, sr->notif.user_data);
...
int sr_start_listening(struct speech_rec *sr)
{
	...
	if (sr->notif.on_speech_begin)
		sr->notif.on_speech_begin(sr->notif.user_data);
